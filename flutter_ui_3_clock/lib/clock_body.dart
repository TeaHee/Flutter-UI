...
import 'package:flutter_ui_3_clock/clock_face.dart';
...
class BellsAndLegsPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final radisus = size.width / 2;
    canvas.save();

    canvas.translate(radisus, radisus);

    // draw the handle
    Path path = new Path();
    path.moveTo(-60.0, -radisus - 10);
    path.lineTo(-50.0, -radisus - 50);
    path.lineTo(50.0, -radisus - 50);
    path.lineTo(60.0, -radisus - 10);

    canvas.drawPath(path, legPaint);

    // draw right bell and left leg
    canvas.rotate(2 * pi / 12);
    drawBellAndLeg(radisus, canvas);
